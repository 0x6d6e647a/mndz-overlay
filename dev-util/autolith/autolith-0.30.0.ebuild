...
src_prepare() {
	...
	ver_test "${host_pv}" -ge "${floor}" ||
		die "host SBCL ${host_pv} is older than Autolith floor ${floor} (need >=dev-lisp/sbcl-2.6.4:=[source]-${floor}[source])"
...
fail() {
	...
	printf 'Install >=dev-lisp/sbcl-2.6.4:=[source] with USE=source; do not use bin/autolith-runtime --install.\n' >&2
	exit 1
}
...
if [[ -z "$sbcl_command" ]]; then
	sbcl_command=$(command -v sbcl 2>/dev/null) || fail "SBCL not found; set AUTOLITH_SBCL or install >=dev-lisp/sbcl-2.6.4:=[source]."
fi
...
src_compile() {
	...
	[[ -d ${gentoo_src} ]] ||
		die "Gentoo SBCL sources missing at ${gentoo_src}; need USE=source on >=dev-lisp/sbcl-2.6.4:=[source]"
